                        ...
                        @else
                            @foreach ($strukturs as $struktur)
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-primary ButtonAksi" style="width: 60px;" onclick="tampilkanModal( 'update', {{ $struktur->id }})"><p>Edit</p></button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('strukturs.storeOrUpdate') }}" method="post" enctype="multipart/form-data" id="editForm">
                    @csrf
                    <input type="hidden" name="formMethod" id="formMethod" value="">
                    <input type="hidden" name="struktur_id" id="struktur_id" value="">
                    <div class="mb-3">
...
<script>
    function tampilkanModal(action, id = null) {
        ...
        if (action === 'store') {
            ...
            $.ajax({
                url: "{{ url('/strukturs/show/') }}" + '/' + id,
                type: 'GET',
                success: function (data) {
                    // Fill the form fields with the existing data
                    $('#struktur_id').val(data.id);
